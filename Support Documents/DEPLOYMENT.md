...
3. Add a mapping in `scripts/gen_app_config.py` (`_ENV_VAR_SOURCES`) so the
   generator injects it into `app-config.json`.
...
6. **Secret hygiene.** Removed committed secrets from git. Introduced
   ...
   `backend/app-config.json` is now gitignored.
...
- **No secrets in git.** `.env` and `backend/app-config.json` are gitignored.
  Only `app-config.template.json` (placeholder) is committed.
- **Rotate anything that was ever committed.** Earlier commits in git history
  contained real secrets (a since-rotated API token, the DB password, and
  `APP_SECRET_KEY`). Treat the DB password and `APP_SECRET_KEY` as compromised
  and rotate them: change the RDS password and update `DB_PASSWORD` in `.env`;
  generate a new `APP_SECRET_KEY` (`python3 -c "import secrets;
  print(secrets.token_hex(32))"`). Redeploy after each change.
- **CORS is locked to the frontend origin**, never `*`.
- **Never echo secret values** in logs, terminal output, or documentation.
